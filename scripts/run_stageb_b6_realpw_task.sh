...
if [[ "${SKIP_EXISTING}" == "1" && -f "${RUN}/stageb6_diagnostics/reports/stageb6_summary.json" ]]; then
  printf 'Real Powderworld B6 task already complete: data_seed=%s split_seed=%s pca_dim=%s out=%s\n' \
...
printf 'Real Powderworld B6 task complete: data_seed=%s split_seed=%s pca_dim=%s out=%s\n' \
  "${DATA_SEED}" "${SPLIT_SEED}" "${PCA_DIM}" "${RUN}/stageb6_diagnostics"
