...
OUT="${OUT:-outputs/stageb_b6_primary_cell_v1_cpu}"
...
HORIZON="${HORIZON:-3}"
MAX_DELTA_SAMPLES="${MAX_DELTA_SAMPLES:-8192}"
MAX_TRAIN_SAMPLES="${MAX_TRAIN_SAMPLES:-2048}"
MAX_STATES_ANALYZE="${MAX_STATES_ANALYZE:-256}"
...
TASK_FILE="${TASK_FILE:-${OUT}/stageb6_primary_cell_tasks.tsv}"
...
if [[ "${N_TASKS}" == "0" ]]; then
  echo "[error] no Stage B.6 primary-cell tasks generated" >&2
  exit 2
fi
...
CMD=(
  ...
  -J "1-${N_TASKS}"
  -j oe
  -o "${SUBMIT_LOG_DIR}/stageb6_primary_cell_v1_array.pbs.log"
  -v "${VARLIST}"
  scripts/stageb_b6_primary_cell_v1_cpu_array.pbs
)
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  JOBID="$("${CMD[@]}")"
  JOBID="${JOBID%% *}"
  printf '[submitted] Stage B.6 primary-cell array job=%s tasks=%s task_file=%s\n' "${JOBID}" "${N_TASKS}" "${TASK_FILE}"
fi
