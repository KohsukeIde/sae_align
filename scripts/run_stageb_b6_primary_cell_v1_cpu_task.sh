...
BOOTSTRAP_SEED="${BOOTSTRAP_SEED:-$((4000 + DATA_SEED * 100 + SPLIT_SEED + PCA_DIM))}"
...
if [[ "${SKIP_EXISTING}" == "1" && -f "${RUN}/stageb6_diagnostics/reports/stageb6_summary.json" ]]; then
  printf 'Stage B.6 primary-cell task already complete: data_seed=%s split_seed=%s pca_dim=%s out=%s\n' \
...
printf 'Stage B.6 primary-cell task complete: data_seed=%s split_seed=%s pca_dim=%s out=%s\n' \
  "${DATA_SEED}" "${SPLIT_SEED}" "${PCA_DIM}" "${RUN}/stageb6_diagnostics"
