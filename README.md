...
behavioral validation. ToyPowderWorld Stage C is stopped. D0 real-Powderworld audit is complete and No-go; it is not a detector-tweak loop. B6R real-Powderworld static-control replication is also a strict No-go / near-miss. Static-weak channel-pair exploration strengthens Framing D-prime but does not establish full Framing D, so AERA implementation remains blocked while the contribution survey is handled separately. See
...
`docs/staged0_v1_cpu_experiment.md`, as well as `docs/stageb6r_preregistration.md`, `docs/stageb6r_v1_cpu_experiment.md`, `docs/static_weak_setup_exploration.md`, and `docs/static_weak_realpw_v2_experiment.md`.
